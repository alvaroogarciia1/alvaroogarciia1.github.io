---
title: Álvaro García
description: Estudiante de Ingeniería Informática | Desarrollador de Software
---
<div style="text-align: right; margin-top: 15px; font-size: 13px;">
  <a href="/" style="text-decoration: none;">EN</a> |
  <strong>ES</strong>
</div>

<div style="display: flex; align-items: center; gap: 40px; margin-top: 30px;">

  <img src="/assets/images/personalPicture.jpg"
       alt="Foto de perfil"
       width="220"
       style="border-radius: 50%;">

  <div>
    <p>
      Soy estudiante de <strong>Ingeniería Informática</strong> en la
      Universidad Politécnica de Madrid (UPM), con un fuerte interés en el
      <strong>desarrollo backend</strong>, la programación de sistemas
      y la creación de software bien estructurado y mantenible.
    </p>

    <p>
      La mayoría de mis proyectos comenzaron como prácticas universitarias y
      evolucionaron hacia retos personales de aprendizaje, donde pongo el foco en
      código limpio, buena documentación y pruebas automatizadas.
    </p>

    <p>
      📌 <strong>Abierto a oportunidades de prácticas y puestos junior</strong>
      en desarrollo de software y backend.
    </p>
  </div>

</div>

---

## 🚀 Proyectos Destacados

### 🧩 Sokoban – Juego de Escritorio en Java
Implementación en Java del clásico juego Sokoban, con interfaz gráfica,
sistema de deshacer movimientos, persistencia de partida y pruebas automatizadas.  
👉 [Ver repositorio](https://github.com/alvaroogarciia1/sokoban)

### 🖥️ MiniShell – Intérprete de Comandos Unix en C
Shell tipo Unix desarrollado en C, que implementa gestión de procesos,
tuberías, redirecciones y manejo de señales.  
👉 [Ver repositorio](https://github.com/alvaroogarciia1/minishell)

---

## 💻 Tecnologías Principales

- **Lenguajes:** Java, C, Python, Dart, Bash  
- **Backend:** Spring Boot, APIs REST, Hibernate  
- **Bases de datos:** PostgreSQL, MySQL, Firebase  
- **Herramientas y DevOps:** Git, GitHub, GitLab, Maven, SonarQube  

---

## 🎓 Experiencia Académica y Complementaria

- Programación de sistemas en C y Linux  
- Proyectos backend en Java y Spring  
- Desarrollo de videojuegos con C# y Unity  
- Análisis estadístico con R en asignaturas académicas  
- Uso de OpenGL para gráficos 3D básicos y visualización geométrica  
- Apache Tomcat, Ant y WSDL en prácticas de servicios web  
- Introducción y uso de Docker en entornos académicos  
- LaTeX para documentación técnica e informes  

---

## 📫 Contacto

- GitHub: [alvaroogarciia1](https://github.com/alvaroogarciia1)  
- LinkedIn: [Álvaro García](https://www.linkedin.com/in/%C3%A1lvaro-garcia-caro-bartolom%C3%A9-71121b346)
